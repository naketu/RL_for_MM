...
class SingleTrader(Trader):
    ...
    def _buy_limit(self, quantity, price):
        order = Order(round(price, 1), round(quantity), 'bid', self) # Rounding price up to 1 digit after the dot 
#         order = Order(price, round(quantity), 'bid', self) # without rounding
        self.orders.append(order)
        self.market.limit_order(order)

    def _sell_limit(self, quantity, price):
        order = Order(round(price, 1), round(quantity), 'ask', self) # Rounding price up to 1 digit after the dot 
#         order = Order(price, round(quantity), 'ask', self) # without rounding
        self.orders.append(order)
        self.market.limit_order(order)
...
class Fundamentalist(SingleTrader):
    ...
    def call(self):
        ...
        pf = round(self.evaluate(divs, rf), 1)  # fundamental price - rounded
#         pf = self.evaluate(divs, rf)  # fundamental price
        p = self.market.price()
        spread = self.market.spread()
        ...
        if random_state > .45:
            ...
            ask_t = round(spread['ask'] * (1 + t_cost), 1) # - rounded
            bid_t = round(spread['bid'] * (1 - t_cost), 1) # - rounded

#             ask_t = spread['ask'] * (1 + t_cost) # - not rounded
#             bid_t = spread['bid'] * (1 - t_cost) # - not rounded
...
class ModelTrader(SingleTrader):
    def __init__(
            self,
            market: ExchangeAgent,
            model,
    ...
    def set_feature_extractor(self, FC):
        self.FC = FC
        
    def get_features(self):
        return self.FC.get_features()
    ...
    def call(self):
        ...
        if self.last_move % self.timing == 0:
            ...
            if len(self.predictions) >= 2:
                if self.predictions[-2] == prediction:
                    self.last_move += 1
                    return
        
            if prediction == 0:
                qty = 2000 / self.market.price()
