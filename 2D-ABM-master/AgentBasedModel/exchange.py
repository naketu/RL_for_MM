...
class TransactionContainer:
    ...
    def append(self, price, qty, side, trader_link):
        self.transactions.append([self.iteration, price, qty, side, trader_link])
...
class Stock(Asset):
    ...
    def _next_dividend(self, div_current: float | int, std: float | int = 2e-3) -> float | int:
...
class ExchangeAgent:
    ...
    def __init__(
        ...
        self.transactions = TransactionContainer()
    ...
    def limit_order(self, order: Order):
        ...
        if order.order_type == 'bid':
            if order.price >= ask:
                order = self.order_book['ask'].fulfill(order, t_cost, transaction_container=self.transactions)
            if order.qty > 0:
            ...
            if order.price <= bid:
                order = self.order_book['bid'].fulfill(order, t_cost, transaction_container=self.transactions)
            if order.qty > 0:
