...
class OrderList:
    ...
    def fulfill(self, order: Order, t_cost: float, transaction_container=None) -> Order:
        if order.order_type == self.order_type:
            raise ValueError(f'Wrong order type! OrderList: {self.order_type}, Order: {order.order_type}')
        ...
        for val in self:
            ...
            if transaction_container:
                transaction_container.append(tmp_price, tmp_qty, order.order_type, order.trader)
